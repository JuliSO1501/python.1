...
@app.route("/items/search/<produto>", methods=["GET"])
def get_produto(produto):
    ...
    try:
        ...
        # Executa o SQL.
        cursor.execute(
            "SELECT * FROM item WHERE item_status!='off' and ( item_name like'%' || ? || '%' or item_description like'%' || ? || '%' or item_ location like'%' || ? || '%');", ( produto,produto,produto,))
